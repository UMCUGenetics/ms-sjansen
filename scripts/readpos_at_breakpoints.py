...
def get_sa_info(read, end = True):
    ...
    if len(satag) > 0:
        ...
        if not read.is_reverse and not orien[sa_orien]:
            ...
            orien = 'FF'
        sa_end = get_sa_end(cigar, sa_start)
                    ...
                    if is_clipped(read):
                        ...
                        if len(read.get_tag('SA').split(",")) == 6:
                            ...
                            if get_SA:
                                ...
                                sa_chr, sa_start, sa_end, orien, sa_side = get_sa_info(read)
...
def find_mate(bamfile, vcf, mean, stdev, df_bp1, df_bp2, winsize):
...
def get_reads_at_breakpoints(bamfile, vcf, winsize = 100, DataName, returnCSV, get_SA = True, get_overlap = False, find_mate = True):
    ...
    df_bp1, df_bp2, dfSA1, dfSA2 = get_reads_SVpos(bamfile, svlist = SV_list, mean= mean, stdev = stdev,  winsize = 100, get_SA = True)
...
def main():
    ...
    get_reads_at_breakpoints(bamfile = args.bamfile, vcf = args.SV_vcf, winsize = args.winsize, DataName = args.DataName, returnCSV = args.returnCSV, \
                             get_SA = args.get_sa,get_overlap = args.get_overlap, find_mate = args.find_mate)
